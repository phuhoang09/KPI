  ...
  public  class BuildingAdminDAO
    {
        ...
        public int Add(EF.Building entity)
        {
            var code = entity.Code.ToUpper();
            List<EF.KPILevel> kpiLevelList = new List<EF.KPILevel>();

            if (_dbContext.Buildings.FirstOrDefault(x => x.Code == code) != null)
            {
                return 2;
            }
        ...
        public bool Update(EF.Building entity)
        {
            var code = entity.Code.ToUpper();
            var item = _dbContext.Buildings.FirstOrDefault(x => x.ID == entity.ID);
            ...
            try
            {
                var SOP = _dbContext.Buildings.Find(entity.ID);
        ...
        public IEnumerable<EF.Building> GetAll()
        {
            return _dbContext.Buildings.ToList();
        }
        public EF.Building GetbyID(int ID)
        {
            return _dbContext.Buildings.FirstOrDefault(x=>x.ID==ID);
        }
        ...
        //    return true;
        //}
        public PagedData<EF.Building> GetPaggedData(int pageNumber = 1, int pageSize = 5)
        {
            List<EF.Building> listData = _dbContext.Buildings.ToList();
