  ...
  public  class TeamAdminDAO
    {
        ...
        public int Add(EF.Team entity)
        {
            var code = entity.Code.ToUpper();
            List<EF.KPILevel> kpiLevelList = new List<EF.KPILevel>();

            if (_dbContext.Teams.FirstOrDefault(x => x.Code == code) != null)
            {
                return 2;
            }
        ...
        public bool Update(EF.Team entity)
        {
            var code = entity.Code.ToUpper();
            var item = _dbContext.Teams.FirstOrDefault(x => x.ID == entity.ID);
            ...
            try
            {
                var SOP = _dbContext.Teams.Find(entity.ID);
        ...
        public IEnumerable<EF.Team> GetAll()
        {
            return _dbContext.Teams.ToList();
        }
        public EF.Team GetbyID(int ID)
        {
        ...
        //    return true;
        //}
        public PagedData<EF.Team> GetPaggedData(int pageNumber = 1, int pageSize = 5)
        {
            List<EF.Team> listData = _dbContext.Teams.ToList();
