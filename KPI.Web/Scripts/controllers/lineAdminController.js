...
var lineAdminController = {
    init: function () {
        lineAdminController.loadData();
        lineAdminController.registerEvent();
    },
    registerEvent: function () {
        $('#search').off('keypress').on('keypress', function (e) {
            if (e.which === 13) {
                lineAdminController.loadData(true);
            }
        });
        $('#box select').off('change').on('change', function (e) {
            var code = $(this).parent().children('.code').text();
            lineAdminController.loadDataKPILevel(true, code);
        });
        //Save KPI
        $('#btnAdd').off('click').on('click', function () {
            lineAdminController.addData();
        ...
        $('.btnUpdate').off('click').on('click', function () {
            //load detail
            lineAdminController.loadDetail(Number($(this).data('id')));
        });
        //save
        $('#btnSaveUpdateModal').off('click').on('click', function () {
            lineAdminController.updateData();
        });
        //delete
        $('.btnDelete').off('click').on('click', function () {
            lineAdminController.deleteData(Number($(this).data('id')));
        });
        //get kpicode
        ...
        //showkpi Modal
        $('#tblline .showTblKPI').off('click').on('click', function () {
            $('#box').toggle(800);
            ...
            lineAdminController.loadDataKPILevel(true, code);
            lineAdminController.loadDataCategory();
        ...
        $('#tblkpilevel tr td:nth-child(2) input').change(function () {
            ...
            lineAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID);
        });
        ...
        $("#btnsavequaterlymodal").off('click').on('click', function (e) {
            ...
            setTimeout(lineAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID)
                , 200);
    ...
    addData: function () {
        var res = lineAdminController.validate();
        if (res === false) {
            return false;
        }
        var mObj = {
    ...
    loadDataKPILevel: function (changePageSize, code) {
        ...
        $.ajax({
            url: '/AdminLine/LoadDataKPILevel',
            type: "GET",
            data: {
                code: code,
                category: category,
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    lineAdminController.pagingKPILevel(response.total, function () {
                        lineAdminController.loadDataKPILevel("", code);
                    }, changePageSize);
                    lineAdminController.registerEvent();
                }
            },
    ...
    loadData: function (changePageSize) {
        ...
        $.ajax({
            url: '/AdminLine/LoadData/',
            type: 'GET',
            data: {
                search: search,
                page: lineConfig.pageIndex,
                pageSize: lineConfig.pageSize
            },
            dataType: 'json',
            beforeSend: function () {
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    var data = response.data;
                    var html = '';
                    var template = $('#tblline-template').html();
                    $.each(data, function (i, item) {
                        html += Mustache.render(template, {
                            No: count,
                            Name: item.Name,
                    ...
                    $('#tblline').html(html);
                    lineAdminController.paging(response.total, function () {
                        lineAdminController.loadData();
                    }, changePageSize);
                    lineAdminController.registerEvent();
                }
    ...
    paging: function (totalRow, callback, changePageSize) {
        var totalPage = Math.ceil(totalRow / lineConfig.pageSize);
