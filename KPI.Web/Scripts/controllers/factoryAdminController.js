...
var factoryAdminController = {
    init: function () {
        factoryAdminController.loadData();
        factoryAdminController.registerEvent();
    },
    registerEvent: function () {
        $('#search').off('keypress').on('keypress', function (e) {
            if (e.which === 13) {
                factoryAdminController.loadData(true);
            }
        });
        $('#box select').off('change').on('change', function (e) {
            var code = $(this).parent().children('.code').text();
            factoryAdminController.loadDataKPILevel(true, code);
        });
        //Save KPI
        $('#btnAdd').off('click').on('click', function () {
            factoryAdminController.addData();
        ...
        $('.btnUpdate').off('click').on('click', function () {
            //load detail
            factoryAdminController.loadDetail(Number($(this).data('id')));
        });
        //save
        $('#btnSaveUpdateModal').off('click').on('click', function () {
            factoryAdminController.updateData();
        });
        //delete
        $('.btnDelete').off('click').on('click', function () {
            factoryAdminController.deleteData(Number($(this).data('id')));
        });
        //get kpicode
        ...
        //showkpi Modal
        $('#tblfactory .showTblKPI').off('click').on('click', function () {
            $('#box').toggle(800);
            ...
            factoryAdminController.loadDataKPILevel(true, code);
            factoryAdminController.loadDataCategory();
        ...
        $('#tblkpilevel tr td:nth-child(2) input').change(function () {
            ...
            factoryAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID);
        });
        ...
        $("#btnsavequaterlymodal").off('click').on('click', function (e) {
            ...
            setTimeout(factoryAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID)
                , 200);
    ...
    addData: function () {
        var res = factoryAdminController.validate();
        if (res === false) {
            return false;
        }
        var mObj = {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result === 1) {
                    ...
                    factoryAdminController.loadData(true);
                    factoryAdminController.resetForm();
                }
                else {
                    Swal.fire({
                        title: 'Error!',
    ...
    updateKPILevel: function (KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID) {
        ...
        $.ajax({
            url: "/AdminFactory/UpdateKPILevel",
            data: JSON.stringify(mObj),
            type: "POST",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
    ...
    loadDataKPILevel: function (changePageSize, code) {
        ...
        $.ajax({
            url: '/AdminFactory/LoadDataKPILevel',
            type: "GET",
            data: {
                code: code,
                category: category,
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    factoryAdminController.pagingKPILevel(response.total, function () {
                        factoryAdminController.loadDataKPILevel("", code);
                    }, changePageSize);
                    factoryAdminController.registerEvent();
                }
            },
    ...
    loadData: function (changePageSize) {
        ...
        $.ajax({
            url: '/AdminFactory/LoadData/',
            type: 'GET',
            data: {
                search: search,
                page: factoryConfig.pageIndex,
                pageSize: factoryConfig.pageSize
            },
            dataType: 'json',
            beforeSend: function () {
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    var data = response.data;
                    var html = '';
                    var template = $('#tblfactory-template').html();
                    $.each(data, function (i, item) {
                        html += Mustache.render(template, {
                            No: count,
                            Name: item.Name,
                    ...
                    $('#tblfactory').html(html);
                    factoryAdminController.paging(response.total, function () {
                        factoryAdminController.loadData();
                    }, changePageSize);
                    factoryAdminController.registerEvent();
                }
    ...
    paging: function (totalRow, callback, changePageSize) {
        var totalPage = Math.ceil(totalRow / factoryConfig.pageSize);
