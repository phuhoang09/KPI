...
var groupAdminController = {
    init: function () {
        groupAdminController.loadData();
        groupAdminController.registerEvent();
    },
    registerEvent: function () {
        $('#search').off('keypress').on('keypress', function (e) {
            if (e.which === 13) {
                groupAdminController.loadData(true);
            }
        });
        $('#box select').off('change').on('change', function (e) {
            var code = $(this).parent().children('.code').text();
            groupAdminController.loadDataKPILevel(true, code);
        });

        //Save KPI
        $('#btnAdd').off('click').on('click', function () {
            groupAdminController.addData();
        ...
        $('.btnUpdate').off('click').on('click', function () {
            //load detail
            groupAdminController.loadDetail(Number($(this).data('id')));
        });
        //save
        $('#btnSaveUpdateModal').off('click').on('click', function () {
            groupAdminController.updateData();
        });
        //delete
        $('.btnDelete').off('click').on('click', function () {
            groupAdminController.deleteData(Number($(this).data('id')));
        });
        //get kpicode
        ...
        //showkpi Modal
        $('#tblgroup .showTblKPI').off('click').on('click', function () {
            $('#box').toggle(800);
            ...
            groupAdminController.loadDataKPILevel(true, code);
            groupAdminController.loadDataCategory();
        ...
        $('#tblkpilevel tr td:nth-child(2) input').change(function () {
            ...
            groupAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID);
        });
        ...
        $("#btnsavequaterlymodal").off('click').on('click', function (e) {
            ...
            setTimeout(groupAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID)
                , 200);
    ...
    addData: function () {
        var res = groupAdminController.validate();
        if (res === false) {
            return false;
        }
        var mObj = {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result === 1) {
                    ...
                    groupAdminController.loadData(true);
                    groupAdminController.resetForm();
                }
                else {
                    Swal.fire({
                        title: 'Error!',
    ...
    deleteData: function (id) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "/AdminGroup/Delete/" + ID,
                    type: "POST",
                    contentType: "application/json;charset=UTF-8",
                    dataType: "json",
                    success: function (result) {
                        //LoadData();
                        //GetPageData(Number($('.page-number.active').text()));
                        Swal.fire({
                            title: 'Success!',
    ...
    loadDataKPILevel: function (changePageSize, code) {
        ...
        $.ajax({
            url: '/AdminGroup/LoadDataKPILevel',
            type: "GET",
            data: {
                code: code,
                category: category,
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    groupAdminController.pagingKPILevel(response.total, function () {
                        groupAdminController.loadDataKPILevel("", code);
                    }, changePageSize);
                    groupAdminController.registerEvent();
                }
            },
    ...
    loadData: function (changePageSize) {
        ...
        $.ajax({
            url: '/AdminGroup/LoadData/',
            type: 'GET',
            data: {
                search: search,
                page: groupConfig.pageIndex,
                pageSize: groupConfig.pageSize
            },
            dataType: 'json',
            beforeSend: function () {
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    var data = response.data;
                    var html = '';
                    var template = $('#tblgroup-template').html();
                    $.each(data, function (i, item) {
                        html += Mustache.render(template, {
                            No: count,
                            Name: item.Name,
                    ...
                    $('#tblgroup').html(html);
                    groupAdminController.paging(response.total, function () {
                        groupAdminController.loadData();
                    }, changePageSize);
                    groupAdminController.registerEvent();
                }
    ...
    paging: function (totalRow, callback, changePageSize) {
        var totalPage = Math.ceil(totalRow / groupConfig.pageSize);
