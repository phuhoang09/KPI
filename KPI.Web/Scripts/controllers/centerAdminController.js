...
var centerAdminController = {
    init: function () {
        centerAdminController.loadData();
        centerAdminController.registerEvent();
    },
    registerEvent: function () {
        $('#search').off('keypress').on('keypress', function (e) {
            if (e.which === 13) {
                centerAdminController.loadData(true);
            }
        });
        $('#box select').off('change').on('change', function (e) {
            var code = $(this).parent().children('.code').text();
            centerAdminController.loadDataKPILevel(true, code);
        });
        //Save KPI
        $('#btnAdd').off('click').on('click', function () {
            centerAdminController.addData();
        ...
        $('.btnUpdate').off('click').on('click', function () {
            //load detail
            centerAdminController.loadDetail(Number($(this).data('id')));
        });
        //save
        $('#btnSaveUpdateModal').off('click').on('click', function () {
            centerAdminController.updateData();
        });
        //delete
        $('.btnDelete').off('click').on('click', function () {
            centerAdminController.deleteData(Number($(this).data('id')));
        });
        //get kpicode
        ...
        //showkpi Modal
        $('#tblcenter .showTblKPI').off('click').on('click', function () {
            $('#box').toggle(800);
            ...
            centerAdminController.loadDataKPILevel(true, code);
            centerAdminController.loadDataCategory();
        ...
        $('#tblkpilevel tr td:nth-child(2) input').change(function () {
            ...
            centerAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID);
        });
        ...
        $("#btnsavequaterlymodal").off('click').on('click', function (e) {
            ...
            setTimeout(centerAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID)
                , 200);
    ...
    addData: function () {
        var res = centerAdminController.validate();
        if (res === false) {
            return false;
        }
        var mObj = {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result === 1) {
                    ...
                    centerAdminController.loadData(true);
                    centerAdminController.resetForm();
                }
                else {
                    Swal.fire({
                        title: 'Error!',
    ...
    updateKPILevel: function (KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID) {
        ...
        $.ajax({
            url: "/AdminCenter/UpdateKPILevel",
            data: JSON.stringify(mObj),
            type: "POST",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
    ...
    loadDataKPILevel: function (changePageSize, code) {
        ...
        $.ajax({
            url: '/AdminCenter/LoadDataKPILevel',
            type: "GET",
            data: {
                code: code,
                category: category,
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    centerAdminController.pagingKPILevel(response.total, function () {
                        centerAdminController.loadDataKPILevel("", code);
                    }, changePageSize);
                    centerAdminController.registerEvent();
                }
            },
    ...
    loadData: function (changePageSize) {
        ...
        $.ajax({
            url: '/AdminCenter/LoadData/',
            type: 'GET',
            data: {
                search: search,
                page: centerConfig.pageIndex,
                pageSize: centerConfig.pageSize
            },
            dataType: 'json',
            beforeSend: function () {
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    var data = response.data;
                    var html = '';
                    var template = $('#tblcenter-template').html();
                    $.each(data, function (i, item) {
                        html += Mustache.render(template, {
                            No: count,
                            Name: item.Name,
                    ...
                    $('#tblcenter').html(html);
                    centerAdminController.paging(response.total, function () {
                        centerAdminController.loadData();
                    }, changePageSize);
                    centerAdminController.registerEvent();
                }
    ...
    paging: function (totalRow, callback, changePageSize) {
        var totalPage = Math.ceil(totalRow / centerConfig.pageSize);
