...
var divisionAdminController = {
    init: function () {
        divisionAdminController.loadData();
        divisionAdminController.registerEvent();
    },
    registerEvent: function () {
        $('#search').off('keypress').on('keypress', function (e) {
            if (e.which === 13) {
                divisionAdminController.loadData(true);
            }
        });
        $('#box select').off('change').on('change', function (e) {
            var code = $(this).parent().children('.code').text();
            divisionAdminController.loadDataKPILevel(true, code);
        });
        //Save KPI
        $('#btnAdd').off('click').on('click', function () {
            divisionAdminController.addData();
        ...
        $('.btnUpdate').off('click').on('click', function () {
            //load detail
            divisionAdminController.loadDetail(Number($(this).data('id')));
        });
        //save
        $('#btnSaveUpdateModal').off('click').on('click', function () {
            divisionAdminController.updateData();
        });
        //delete
        $('.btnDelete').off('click').on('click', function () {
            divisionAdminController.deleteData(Number($(this).data('id')));
        });
        //get kpicode
        ...
        //showkpi Modal
        $('#tbldivision .showTblKPI').off('click').on('click', function () {
            $('#box').toggle(800);
            ...
            divisionAdminController.loadDataKPILevel(true, code);
            divisionAdminController.loadDataCategory();
        ...
        $('#tblkpilevel tr td:nth-child(2) input').change(function () {
            ...
            divisionAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID);
        });
        ...
        $("#btnsavequaterlymodal").off('click').on('click', function (e) {
            ...
            setTimeout(divisionAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID)
                , 200);
    ...
    addData: function () {
        var res = divisionAdminController.validate();
        if (res === false) {
            return false;
        }
        var mObj = {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result === 1) {
                    ...
                    divisionAdminController.loadData(true);
                    divisionAdminController.resetForm();
                }
                else {
                    Swal.fire({
                        title: 'Error!',
    ...
    updateKPILevel: function (KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID) {
        ...
        $.ajax({
            url: "/AdminDivision/UpdateKPILevel",
            data: JSON.stringify(mObj),
            type: "POST",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
    ...
    loadDataKPILevel: function (changePageSize, code) {
        ...
        $.ajax({
            url: '/AdminDivision/LoadDataKPILevel',
            type: "GET",
            data: {
                code: code,
                category: category,
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    divisionAdminController.pagingKPILevel(response.total, function () {
                        divisionAdminController.loadDataKPILevel("", code);
                    }, changePageSize);
                    divisionAdminController.registerEvent();
                }
            },
    ...
    loadData: function (changePageSize) {
        ...
        $.ajax({
            url: '/AdminDivision/LoadData/',
            type: 'GET',
            data: {
                search: search,
                page: divisionConfig.pageIndex,
                pageSize: divisionConfig.pageSize
            },
            dataType: 'json',
            beforeSend: function () {
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    var data = response.data;
                    var html = '';
                    var template = $('#tbldivision-template').html();
                    $.each(data, function (i, item) {
                        html += Mustache.render(template, {
                            No: count,
                            Name: item.Name,
                    ...
                    $('#tbldivision').html(html);
                    divisionAdminController.paging(response.total, function () {
                        divisionAdminController.loadData();
                    }, changePageSize);
                    divisionAdminController.registerEvent();
                }
    ...
    paging: function (totalRow, callback, changePageSize) {
        var totalPage = Math.ceil(totalRow / divisionConfig.pageSize);
