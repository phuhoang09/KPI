$(document).ready(function () {
    departmentAdminController.init();
});
...
var departmentAdminController = {
    init: function () {
        departmentAdminController.loadData();
        departmentAdminController.registerEvent();
    },
    registerEvent: function () {
        $('#search').off('keypress').on('keypress', function (e) {
            if (e.which === 13) {
                departmentAdminController.loadData(true);
            }
        });
        $('#box select').off('change').on('change', function (e) {
            var code = $(this).parent().children('.code').text();
            departmentAdminController.loadDataKPILevel(true, code);
        });
        //Save KPI
        $('#btnAdd').off('click').on('click', function () {
            departmentAdminController.addData();
        ...
        $('.btnUpdate').off('click').on('click', function () {
            //load detail
            departmentAdminController.loadDetail(Number($(this).data('id')));
        });
        //save
        $('#btnSaveUpdateModal').off('click').on('click', function () {
            departmentAdminController.updateData();
        });
        //delete
        $('.btnDelete').off('click').on('click', function () {
            departmentAdminController.deleteData(Number($(this).data('id')));
        });
        //get kpicode
        ...
        $('#tbldepartment .showTblKPI').off('click').on('click', function () {
            ...
            departmentAdminController.loadDataKPILevel(true, code);
            departmentAdminController.loadDataCategory();
        });
        //update kpilevel
        $('#tblkpilevel tr td:nth-child(2) input').change(function () {
            ...
            departmentAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID);
        });
        ...
        $("#btnsavequaterlymodal").off('click').on('click', function (e) {
            ...
            setTimeout(departmentAdminController.updateKPILevel(KPICode, CodeKPILevel, UserCheck, Checked, WeeklyChecked, MonthlyChecked, QuaterlyChecked, YearlyChecked, Weekly, Monthly, Quaterly, Yearly, TimeCheck, TableID)
                , 200);
    ...
    addData: function () {
        var res = departmentAdminController.validate();
        if (res === false) {
            return false;
        }
        var mObj = {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result === 1) {
                    ...
                    departmentAdminController.loadData(true);
                    departmentAdminController.resetForm();
                }
                else {
                    Swal.fire({
                        title: 'Error!',
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    departmentAdminController.pagingKPILevel(response.total, function () {
                        departmentAdminController.loadDataKPILevel("", code);
                    }, changePageSize);
                    departmentAdminController.registerEvent();
                }
            },
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    departmentAdminController.paging(response.total, function () {
                        departmentAdminController.loadData();
                    }, changePageSize);
                    departmentAdminController.registerEvent();
                }
